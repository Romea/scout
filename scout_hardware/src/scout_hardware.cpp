...
ScoutHardwareBase::~ScoutHardwareBase() {
  // force deactive when interface has not been deactivated by controller
  // manager but by ctrl-c
...
hardware_interface::return_type ScoutHardwareBase::connect_() {
  // RCLCPP_ERROR(rclcpp::get_logger("ScoutHardware"), "Init communication with
  // robot"); To be implememented
...
hardware_interface::return_type ScoutHardwareBase::disconnect_() {
  // RCLCPP_ERROR(rclcpp::get_logger("ScoutHardware"), "Close communication with
  // robot"); To be implememented
...
hardware_interface::return_type ScoutHardwareBase::load_info_(
  ...
  try {
    ...
    // auto front_wheel_radius_ = get_parameter<float>(hardware_info,
    // "front_wheel_radius"); auto rear_wheel_radius_ =
    // get_parameter<float>(hardware_info, "rear_wheel_radius");
...
//-----------------------------------------------------------------------------
void ScoutHardwareBase::send_null_command_() { robot_.SetMotionCommand(0, 0); }
...
#endif
{
  ...
  try {
    ...
    //   actuator.actuator_hs_state[FRONT_LEFT_SPINNING_MOTOR_ID_].rpm * ? /
    //   wheel_radius_ ;
    // front_right_wheel_angular_speed_measure_ =
    //   actuator.actuator_hs_state[FRONT_RIGHT_SPINNING_MOTOR_ID_].rpm * ? /
    //   wheel_radius_ ;
    // rear_left_wheel_angular_speed_measure_ =
    //   actuator.actuator_hs_state[REAR_LEFT_SPINNING_MOTOR_ID_].rpm * ? /
    //   wheel_radius_;
...
#endif
{
  ...
  auto left_linear_speed_command =
      0.5 * (front_left_wheel_angular_speed_command_ * wheel_radius_ +
             rear_left_wheel_angular_speed_command_ * wheel_radius_);
  auto right_linear_speed_command =
      0.5 * (front_right_wheel_angular_speed_command_ * wheel_radius_ +
             rear_right_wheel_angular_speed_command_ * wheel_radius_);

  auto linear_speed_command =
      0.5 * (left_linear_speed_command + right_linear_speed_command);
  auto angular_speed_command =
      (right_linear_speed_command - left_linear_speed_command) / track_;
...
void ScoutHardwareBase::get_hardware_command_() {
  core::HardwareCommand4WD command =
      hardware_interface_->get_hardware_command();
...
PLUGINLIB_EXPORT_CLASS(romea::ros2::ScoutMiniHardware,
                       hardware_interface::SystemInterface)
PLUGINLIB_EXPORT_CLASS(romea::ros2::ScoutV2Hardware,
                       hardware_interface::SystemInterface)
